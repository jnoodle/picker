...
export default function useCellClassName<DateType>({
  ...
  function getClassName(currentDate: DateType) {
    ...
    const isViewEdgeStart = !isInView(prevDate) || isRangeEnd(prevDate);
    const isViewEdgeEnd = !isInView(nextDate) || isRangeStart(nextDate);
    ...
    return {
      ...
      // Range Edge
      [`${cellPrefixCls}-range-edge-start`]: isViewEdgeStart && isInView(currentDate), // selected range calendar month start indicator for style
      [`${cellPrefixCls}-range-edge-end`]: isViewEdgeEnd && isInView(currentDate), // selected range calendar month end indicator for style
